...
import { isGroupBanned } from '../plugins/bangroup.js';
...
import { storeGroupMessage, handleGroupMessageRevocation } from '../plugins/groupantidelete.js';
...
import { handleForcaGuess } from '../plugins/forca.js';
import { handleShiritoriMove } from '../plugins/shiritori.js';
import { handleGarticGuess } from '../plugins/gartic.js';
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        if (isGroup && await isGroupBanned(chatId))
            return;
        ...
        if (message.message?.protocolMessage?.type === 0) {
            ...
            await handleGroupMessageRevocation(sock, message);
            return;
        }
        await storeMessage(sock, message);
        await storeGroupMessage(sock, message);
        ...
        const altSenderId = message.key.participantAlt || message.key.remoteJidAlt;
        // Resolve @lid to real JID if possible. Baileys puts the other
        // identity form (phone JID when the primary is a LID) directly on
        // the message key, which is more reliable than a contacts cache that
        // may not be populated yet (e.g. right after reconnecting).
        let senderId = rawSenderId;
        if (rawSenderId?.includes('@lid')) {
            if (altSenderId?.includes('@s.whatsapp.net')) {
                senderId = altSenderId;
            }
            else if (sock.store?.contacts) {
                ...
                if (stickers[hash]) {
                    ...
                    if (foundCommand) {
                        ...
                        if (foundCommand.ownerOnly && !message.key.fromMe && !senderIsOwnerOrSudo) {
                            return await sock.sendMessage(chatId, {
                                text: t('system.ownerOrSudoOnly'),
                                ...channelInfo
                            }, { quoted: message });
                        }
                        if (foundCommand.groupOnly && !isGroup) {
                            return await sock.sendMessage(chatId, {
                                text: t('system.groupOnly'),
                                ...channelInfo
                            }, { quoted: message });
                        }
                        ...
                        try {
                            ...
                            await sock.sendMessage(chatId, {
                                text: t('system.stickerCommandError', { message: error.message }),
                                ...channelInfo
                            }, { quoted: message });
                        }
                    }
        ...
        if (userBanned && !userMessage.startsWith('.unban')) {
            if (Math.random() < 0.1) {
                ...
                await sock.sendMessage(chatId, {
                    text: t('system.banned'),
                    ...channelInfo
                });
            }
            return;
        }
        if (/^[1-9]$/.test(userMessage) || userMessage === 'surrender' || userMessage === 'desistir' || userMessage === 'give up') {
            await handleTicTacToeMove(sock, chatId, senderId, userMessage);
            return;
        }
        if (await handleForcaGuess(sock, chatId, senderId, userMessage))
            return;
        if (await handleShiritoriMove(sock, chatId, senderId, userMessage))
            return;
        if (await handleGarticGuess(sock, chatId, senderId, userMessage))
            return;
        if (!message.key.fromMe) {
        ...
        if (command.strictOwnerOnly) {
            ...
            if (!message.key.fromMe && !isOwnerOnly(senderId)) {
                return await sock.sendMessage(chatId, {
                    text: t('system.botOwnerOnlySudoNote'),
                    ...channelInfo
                }, { quoted: message });
            }
        }
        ...
        try {
            ...
            console.error(error.stack);
            await sock.sendMessage(chatId, {
                text: t('system.commandError', { message: error.message }),
                ...channelInfo
            }, { quoted: message });
            const errorLog = {
