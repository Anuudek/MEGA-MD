...
process.on('SIGINT', async () => {
    ...
    // Baileys writes session key files (device lists, identity keys,
    // app-state-sync) with their own independent async fs writes, often
    // still in flight right after connecting. Exiting immediately here cuts
    // those off mid-write, which is how the session ends up missing
    // everything but creds.json after a restart. Give them a moment first.
