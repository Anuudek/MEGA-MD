...
app.get('/', (req, res) => {
    if (!botState.connected && botState.qr) {
        return res.send(`
        <!DOCTYPE html>
        <html lang="pt-BR">
        <head>
            ...
            <title>Parear ${packageInfo.name}</title>
            <style>
                ...
                .desc { color: #94a3b8; margin: 0 0 20px 0; font-size: 0.85rem; }
                .qr-frame { background: white; padding: 12px; border-radius: 16px; display: inline-block; }
                .qr-frame img { display: block; width: 220px; height: 220px; }
                .hint { color: #64748b; font-size: 0.75rem; margin-top: 16px; }
            </style>
        </head>
        <body>
            <div class="container">
                <h1>Escaneie pra conectar</h1>
                <p class="desc">WhatsApp → Aparelhos conectados → Conectar um aparelho</p>
                <div class="qr-frame"><img src="/qr-image.png?t=${Date.now()}" alt="QR code de pareamento" /></div>
                <p class="hint">Atualiza sozinho a cada 8s enquanto não conectar</p>
            </div>
        </body>
        </html>
        `);
...
app.get('/qr-image.png', async (req, res) => {
    ...
    try {
        const buffer = await QRCode.toBuffer(botState.qr, { width: 440 });
